...
$query = "SELECT * FROM doctorsignup WHERE fullname = '$fullname' ";
    $result = mysqli_query($db, $query);
    $user = mysqli_fetch_assoc($result);

    if($user){
        // if user with same  name already exixts
...
if (!empty($_POST['phonenumber'])) {
    ...
    $query = "SELECT * FROM doctorsignup WHERE phonenumber = '$phonenumber' ";
    $result = mysqli_query($db, $query);
    $user = mysqli_fetch_assoc($result);

    if($user){
        // if user with same phone number already exixts
...
if(count($signup_errors) > 0){
    ...
    $query = "INSERT INTO doctorsignup (fullname,phonenumber,email, password) VALUES('$fullname','$phonenumber','$email', '$password')";
